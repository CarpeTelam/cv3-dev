...
if (
  ...
  console.log("Please run `cv3-dev setup` to create the proper config files.");
  process.exit(1);
}
...
function getTemplateCallback(error, response, body) {
  ...
  fs.readFile(filePath, "utf8", function (error, data) {
    const form = {
      action: "EditTemplate",
      locked:
        json.locked && json.locked.locked_status
          ? json.locked.locked_status
          : "",
      stylesheet_locked:
        json.stylesheet_locked && json.stylesheet_locked.locked_status
          ? json.stylesheet_locked.locked_status
          : "",
      title: json.meta && json.meta.title ? json.meta.title : "",
      description:
        json.meta && json.meta.description ? json.meta.description : "",
...
function setJavascriptCallback() {
...
function setStylesheetCallback() {
  // needs error feedback
...
function getModifiedFiles(path) {
  ...
  const filter = (filePath) =>
    store.timestamp < moment(fs.statSync(filePath).mtime).unix();
  try {
...
function update() {
  return (() => {
    const files = [
      ...getModifiedFiles(filesCodePath),
      ...getModifiedFiles(templatePath),
    ];

    const fileCountText = `${files.length} File${files.length > 1 ? "s" : ""}`;
    console.log(
      `${fileCountText} to update: ${JSON.stringify(files, null, 2)}`
    );
